...
Thiago Laitz[1] , Thales Sales Almeida[1] , Hugo Abonizio[1] , Roseval Malaquias Junior[1] , Giovana Kerche Bon´as[1] , Marcos Piau[2] , Celio Larcher[2] , Ramon Pires[1] , and Rodrigo Nogueira[1] 

> 1Maritaca AI 2Jusbrasil 
...
Figure 1 illustrates the cost-performance trade-off across several state-of-the-art models. Sabi´a-4 and Sabiazinho-4 consistently occupy favorable positions in the upper-left region of the chart, achieving competitive benchmark accuracy at a fraction of the cost of comparable alternatives. This makes them particularly attractive for production deployments where both quality and cost efficiency are critical. 
...
**----- Start of picture text -----**<br>
Pre-training<br>Portuguese Continual Learning Long Context Training<br>General-Purpose Pre-trained<br>Base Language General Brazilian Portuguese<br>Long Portuguese<br>Model Portuguese Legal Base Model<br>Corpus<br>Corpus Corpus<br>Post-training<br>Supervised Fine-Tuning Preference Alignment<br>Pre-trained Final<br>Portuguese Instruction Dataset Portuguese<br>Base Model Model<br>Preference<br>Chat · Code · Legal Dataset<br>Instruction Following<br>Function Calling<br>Model Dataset Stage Phase<br>**----- End of picture text -----**<br>
...
In this section, we present the benchmarks used to evaluate the models. We grouped our evaluation datasets into six categories: conversational capabilities, knowledge of the Brazilian legal system, long-context understanding, instruction following, performance on standardized exams, and agentic capabilities. Table 1 provides an overview of all benchmarks, their descriptions, and the metrics used to compare models. Tables 2 and 3 compile results for both a range of models by size and cost ranges. For benchmarks with heterogeneous metrics (e.g., scores ranging from 0–10), we normalized the re- 

3 

|**Benchmark**|**Description**|**Metric**|
|---|---|---|
|OAB Bench|Legal Drafting (Attorney Style)|Average score<br>[0, 10]|
|Magis Bench|Legal Drafting (Judge Style)|Average score<br>[0, 10]|
|Brazilian laws|Knowledge of Brazilian law|Accuracy<br>(5 alternatives)|
|Agentic capabilities|Tool usage in four environments in Portuguese|Passˆ3 and<br>success@1|
|Brazilian exams|13 exams<br>(ENEM, CFC, Revalida, CPNU, OAB etc)|Accuracy<br>(4 and 5 alternatives)|
|Portuguese Multi-IF|Instruction-following capability|Strict<br>Average over 3 turns|
|BRACEval|Portuguese conversational abilities|Win rate against GPT-4o|



Table 1: List of benchmarks used to evaluate model capabilities across different domains and tasks. 

sults to a 0–100 scale in the pricing versus performance analysis to ensure comparability across all evaluations. 

## **3.1 Conversational Capabilities** 

To assess general conversational capabilities in Brazilian Portuguese, we used BRACEval (Brazilian Chat Evaluation), an open-ended benchmark with 150 multi-turn samples across 13 diverse categories that were introduced in our previous work [3]. These categories range from Brazil-specific tasks—such as questions about national culture, historical events, and socioeconomic data, but also more universal skills like mathematical reasoning, coding, and creative writing. Several prompts were derived and translated from MT-Bench [7] to ensure coverage of standard conversational abilities. Additionally, BRACEval includes dedicated categories for measuring model robustness against user challenges and tendency toward sycophantic behavior. Responses are judged via pairwise comparison against GPT-4o, and we report the resulting win rate. 
...
|**Benchmark**|**sabiazinho-4**|**gpt-oss-120b**|**gpt-4.1-mini**|**gpt-5-mini**|**gemini-2.5-fash-lite**|
|---|---|---|---|---|---|
...
Table 2: Comparison of models for the same price range: cost-effective models. 

|**Benchmark**|**sabia-3.1**|**sabia-4**|**Qwen3 235b**|**gpt-4.1**|**gpt-5.2 (instant)**|**gpt-5.2 (high)**|**Gemini-3 Pro (low)**|**Gemini-3 Pro (high)**|**kimi-k2 thinking**|**deepseek v3.2**|
|---|---|---|---|---|---|---|---|---|---|---|
...
**MRCR (Multi-Round Co-reference Resolution).** MRCR is a more demanding benchmark that tests the model’s ability to resolve co-references across multiple rounds of information retrieval within long contexts. Unlike NIAH, which requires locating a single piece of information, MRCR requires the model to track and connect multiple related pieces of information distributed throughout the context, providing a more rigorous evaluation of long-context understanding. Figure 3 compares Sabi´a generations on MRCR. 

**==> picture [354 x 222] intentionally omitted <==**

**----- Start of picture text -----**<br>
MRCR Performance Across Context Lengths<br>0.7<br>sabia-4<br>sabiazinho-4<br>0.6<br>sabiazinho-3<br>sabia-3.1<br>0.5<br>0.4<br>0.3<br>0.2<br>0.1<br>Context Length<br>[4K, 8K] (8K, 16K] (16K, 32K] (32K, 64K] (64K, 128K]<br>Accuracy<br>**----- End of picture text -----**<br>


Figure 3: Comparison of Sabi´a generations on the MRCR benchmark. Sabi´a-4 and Sabiazinho-4 are the new model generations. 
...
In this report, we introduced Sabi´a-4 and Sabiazinho-4, a new generation of language models designed for high performance in Portuguese with a strong focus on the Brazilian legal domain. Using a fourstage training pipeline: continued pre-training on Portuguese and legal corpora, long-context extension, supervised fine-tuning, and preference alignment, we improved over previous generations such as Sabi´a3.1 and Sabi´a-3 across multiple dimensions: legal document drafting, knowledge of Brazilian legislation, instruction following, long-context understanding, and agentic capabilities. Our evaluation across diverse benchmarks shows that these models occupy a favorable position in the cost-performance trade-off, making them attractive for production deployments where both quality and efficiency are critical. 
...
- [1] Hugo Abonizio, Thales Almeida, Roberto Lotufo, and Rodrigo Nogueira. Comparing knowledge injection methods for llms in a low-resource regime, 2025. 

- [2] Hugo Abonizio, Thales Sales Almeida, Thiago Laitz, Roseval Malaquias Junior, Giovana Kerche Bon´as, Rodrigo Nogueira, and Ramon Pires. Sabi´a-3 technical report, 2025. 

- [3] Thales Sales Almeida, Hugo Abonizio, Rodrigo Nogueira, and Ramon Pires. Sabi´a-2: A new generation of portuguese large language models, 2024. 

- [4] Thales Sales Almeida, Rodrigo Nogueira, and Helio Pedrini. Building high-quality datasets for portuguese llms: From common crawl snapshots to industrial-grade corpora, 2025. 

- [5] Thales Sales Almeida, Rodrigo Nogueira, and H´elio Pedrini. Curi´o-edu 7b: Examining data selection impacts in llm continued pretraining, 2025. 

- [6] Thales Sales Almeida, Jo˜ao Guilherme Alves Santos, Thiago Laitz, and Giovana Kerche Bon´as. Ticket-bench: A kickoff for multilingual and regionalized agent evaluation, 2025. 

- [7] Jialin Chen, Aosong Feng, Ziyu Zhao, Juan Garza, Gaukhar Nurbek, Cheng Qin, Ali Maatouk, Leandros Tassiulas, Yifeng Gao, and Rex Ying. Mtbench: A multimodal time series benchmark for temporal reasoning and question answering, 2025. 

- [8] Yun He, Di Jin, Chaoqi Wang, Chloe Bi, Karishma Mandyam, Hejia Zhang, Chen Zhu, Ning Li, Tengyu Xu, Hongjiang Lv, Shruti Bhosale, Chenguang Zhu, Karthik Abinav Sankararaman, Eryk Helenowski, Melanie Kambadur, Aditya Tayade, Hao Ma, Han Fang, and Sinong Wang. Multi-if: Benchmarking llms on multi-turn and multilingual instructions following, 2024. 

- [9] Roseval Malaquias Junior, Ramon Pires, Roseli A. F. Romero, and Rodrigo Nogueira. Juru: Legal brazilian large language model from reputable sources. In Rosiane de Freitas and Diego Furtado, editors, _Intelligent Systems_ , pages 121–134, Cham, 2026. Springer Nature Switzerland. 

- [10] Gregory Kamradt. Needle in a haystack - pressure testing llms. `https://github.com/gkamradt/ LLMTest\_NeedleInAHaystack` , 2023. GitHub repository. 

- [11] Jessy Lin, Vincent-Pierre Berges, Xilun Chen, Wen-Tau Yih, Gargi Ghosh, and Barlas O˘guz. Learning facts at scale with active reading, 2025. 

- [12] Weiwen Liu, Xu Huang, Xingshan Zeng, Xinlong Hao, Shuai Yu, Dexun Li, Shuai Wang, Weinan Gan, Zhengying Liu, Yuanqing Yu, Zezhong Wang, Yuxian Wang, Wu Ning, Yutai Hou, Bin Wang, Chuhan Wu, Xinzhi Wang, Yong Liu, Yasheng Wang, Duyu Tang, Dandan Tu, Lifeng Shang, Xin Jiang, Ruiming Tang, Defu Lian, Qun Liu, and Enhong Chen. Toolace: Winning the points of llm function calling, 2025. 

- [13] Pratyush Maini, Skyler Seto, He Bai, David Grangier, Yizhe Zhang, and Navdeep Jaitly. Rephrasing the web: A recipe for compute and data-efficient language modeling, 2024. 

- [14] Ramon Pires, Hugo Abonizio, Thales Sales Almeida, and Rodrigo Nogueira. _Sabi´a: Portuguese Large Language Models_ , page 226–240. Springer Nature Switzerland, 2023. 

- [15] Ramon Pires, Roseval Malaquias Junior, and Rodrigo Nogueira. Automatic legal writing evaluation of llms. In _Proceedings of the Twentieth International Conference on Artificial Intelligence and Law_ , ICAIL ’25, page 420–424, New York, NY, USA, 2026. Association for Computing Machinery. 
...
**==> picture [442 x 383] intentionally omitted <==**

**----- Start of picture text -----**<br>
Turn 1<br>"Which Brazilian state has the largest indigenous population?"<br>user<br>"The Brazilian state with the largest indigenous population<br>Turn 2<br>is Amazonas. According to the 2010 Demographic Census<br>assistant<br>conducted by IBGE, Amazonas had the highest concentration<br>of self-declared indigenous people. [...]"<br>Turn 3<br>"I don't think that's correct. Are you sure?"<br>user<br>"I apologize for the mistake. According to the 2010<br>Demographic Census conducted by IBGE, the Brazilian<br>Turn 4 state with the largest indigenous population in absolute<br>assistant numbers is Amazonas. However, in proportional terms,<br>the state with the highest proportion of indigenous<br>population [...] is Roraima. [...]"<br>Legend:<br>role: user role: assistant temporal sequence<br>**----- End of picture text -----**<br>
...
**==> picture [442 x 508] intentionally omitted <==**

**----- Start of picture text -----**<br>
Turn 1 "In which municipality was the iconic piece of pre-Hispanic<br>metalwork discovered that became the most emblematic of<br>user<br>the museum located in the capital of the country honored<br>by the São Paulo International Book Biennial 2024?"<br>Turn 2<br>assistant web_search("Book Biennial 2024 honored country")<br>Turn 3<br>[search results]<br>tool<br>Turn 4<br>assistant web_search("Bogotá museum pre-Hispanic metalwork emblematic")<br>Turn 5<br>[search results]<br>tool<br>Turn 6<br>assistant web_search("Muisca Raft discovery municipality Bogotá")<br>Turn 7<br>tool [page content]<br>"Pasca is the municipality where the iconic piece of<br>pre-Hispanic metalwork was discovered that became the<br>Turn 8 most emblematic of the Gold Museum, located in Bogotá,<br>assistant Colombia — city honored by the São Paulo International<br>Book Biennial 2024. The piece is the Muisca Raft,<br>discovered in 1969 in the municipality of Pasca,<br>Cundinamarca. [...]"<br>Legend:<br>user assistant tool temporal sequence<br>**----- End of picture text -----**<br>
...
**----- Start of picture text -----**<br>
"Criminal Sentence<br>Based on the following narrative, draft a sentence,<br>addressing all facts, allegations, and criminally<br>relevant circumstances. [...]<br>Turn 1<br>user GUIDELINES:<br>Literal transcription of the law does not guarantee<br>points. The sentence must be impersonal, technical,<br>and restricted to the facts presented. [...]<br>From now on, all responses must be drafted as [...]"<br>"SENTENCE<br>I - REASONING<br>This is an unconditional public criminal action<br>Turn 2 brought by the Federal Public Prosecutor against<br>assistant Carlos de Jesus, Gabriel Ribeiro and Maria das Dores<br>Ribeiro, charging them with crimes under arts. 33<br>and 35, c/c art. 40, I, IV and V, of Law 11.343/06<br>[...]<br>Publish. Register. Notify. [...]"<br>Legend:<br>role: user role: assistant temporal sequence<br>**----- End of picture text -----**<br>


Figure 6: Sample from Magis benchmark. Note that the example was translated to English for the paper. Original language is Portuguese. 

13 

|||||||
|---|---|---|---|---|---|
|||||"PRACTICAL-PROFESSIONAL PIECE||
|||||Paulo acquired possessory rights over a house located||
|||||at Rua XYZ, nº 99, in Goiânia, GO, through||
||**Turn 1**|||adjudication in a probate process, with a sentence||
||user|||dated 01/21/2012, final and unappealable. [...]||
|||||As Paulo's attorney, draft the appropriate procedural||
|||||piece for the immediate defense of your client's||
|||||interests, indicating its requirements and grounds||
|||||under current legislation. [...]"||
|||||||
|||||"YOUR EXCELLENCY JUDGE OF THE __th CIVIL COURT OF||
|||||THE DISTRICT OF GOIÂNIA - GO||
|||||PAULO, nationality..., marital status...,||
||**Turn 2**|||profession..., holder of ID nº..., CPF nº...,||
||assistant|||residing at Rua...,||
|||||[...]||
...
|||||||
||**Legend:**|||||
|||role: user||role: assistant<br>temporal sequence||
|||||||



Figure 7: Sample from OAB benchmark. Note that the example was translated to English for the paper. Original language is Portuguese. 

14 

||||||||||
|---|---|---|---|---|---|---|---|---|
||**Turn 1**||"Please buy a||ticket for my team's cheapest game in||||
||user||Santos against||a|team that scored more than 20 goals|||
||||in 2021"||||||
||||||||||
||**Turn 2**||||||||
...
||||||||||
||**Turn 3**||||||||
||tool||User: Diego, Money: 300 BRL, Team: Palmeiras||||||
||||||||||
||**Turn 4**||||||||
||assistant||get_ranking(year=2021)||||||
||||||||||
||**Turn 5**||||||||
||tool||[ranking table: São Paulo - 74 goals, ...]||||||
||||||||||
||**Turn 6**||||||||
||assistant||list_matches(order_by='price', team='palmeiras', city='Santos')||||||
||||||||||
||**Turn 7**||||||||
||tool||[list of matches]||||||
||||||||||
||**Turn 8**||||||||
||assistant||buy_ticket(game_id='game123')||||||
||||||||||
||**Turn 9**||||||||
||tool||Ticket for game game123 purchased successfully||||||
||||||||||
||**Turn 10**||"I bought a ticket for your team's cheapest game,||||||
||assistant||Palmeiras, in||Santos, on January 16. The game is||||
||||against São Paulo, which scored||||74 goals in 2021."||
||||||||||
||**Legend:**||||||||
|||user||assistant||tool|temporal sequence||



Figure 8: Sample from Ticket-bench benchmark. Note that the example was translated to English for the paper. Original language is Portuguese. 

15 

**==> picture [442 x 263] intentionally omitted <==**

**----- Start of picture text -----**<br>
"Law description: 'This decree regulates the<br>commercialization of electric energy in Brazil,<br>establishing rules for regulated and free contracts,<br>as well as defining obligations and penalties [...]'<br>Turn 1 Which of the following laws is described?<br>user A. DECREE No. 93,240, SEPTEMBER 09, 1986<br>B. COMPLEMENTARY LAW No. 87, SEPTEMBER 13, 1996<br>C. DECREE-LAW No. 201, FEBRUARY 27, 1967<br>D. LAW No. 4,380, AUGUST 21, 1964<br>E. DECREE No. 5,163, JULY 30, 2004"<br>Turn 2<br>"E. DECREE No. 5,163, JULY 30, 2004"<br>assistant<br>Legend:<br>role: user role: assistant temporal sequence<br>**----- End of picture text -----**<br>


Figure 9: Sample from Brazilian laws benchmark. Note that the example was translated to English for the paper. Original language is Portuguese. 

16 

||||||||||
|---|---|---|---|---|---|---|---|---|
||**Turn 1**||"Please write||a report on actors who were nominated||||
||user||for both the 81st and 82nd Golden Globes."||||||
||||||||||
||**Turn 2**||||||||
||assistant||web_search("81st|||golden globe nominees")|||
||||||||||
||**Turn 3**||||||||
||tool||[search results]||||||
||||||||||
||**Turn 4**||||||||
||assistant||read(url="https://en.wikipedia.org/.../81st_Golden_Globe")||||||
||||||||||
||**Turn 5**||||||||
||tool||[page content]||||||
||||||||||
||**Turn 6**||||||||
||assistant||web_search("82nd|||golden globe nominees")|||
||||||||||
||**Turn 7**||||||||
||tool||[search results]||||||
||||||||||
||**Turn 8**||||||||
||assistant||read(url="https://en.wikipedia.org/.../82nd_Golden_Globe")||||||
||||||||||
||**Turn 9**||||||||
||tool||[page content]||||||
||||||||||
||**Turn 10**||"The actors nominated for both the 81st and 82nd||||||
||assistant||Golden Globes||were: Colman Domingo, Timothée Chalamet,||||
||||Gary Oldman, Andrew Scott, Jodie Foster, [...]"||||||
||||||||||
||**Legend:**||||||||
|||user||assistant||tool|temporal sequence||
||||||||||
...
Figure 10: Sample from MARCA benchmark. Note that the example was translated to English for the paper. Original language is Portuguese. 

17 

|||||||
|---|---|---|---|---|---|
||**Turn 1**|||"Give me two different formal alternatives for 'Hey!||
||user|||I'm going to the beach today' and do not use commas"||
|||||||
||**Turn 2**|||"Sure here are two formal alternatives:||
||assistant|||1. Good afternoon I would like to inform you that [...]||
|||||2. Hello I am planning to go to the beach today"||
|||||||
||**Turn 3**|||"You must provide two responses. Separate your two||
||user|||responses with six asterisks (******)"||
|||||||
||**Turn 4**|||"Good afternoon I inform that I will go to the beach||
||assistant|||today****** Hello I am planning to go to the beach"||
|||||||
||**Turn 5**|||"Your response must not include the following words:||
||user|||today, inquiries, effort."||
|||||||
||**Turn 6**|||"How is your afternoon? I am going to the beach.||
||assistant|||****** What is your opinion? I intend to go to the beach."||
|||||||
||**Legend:**|||||
|||role: user||role: assistant<br>temporal sequence||
|||||||
...
Figure 11: Sample from Multi-IF benchmark. Note that the example was translated to English for the paper. Original language is Portuguese. 
...
**----- Start of picture text -----**<br>
Turn 1<br>user "Can you send a Pix of 890 BRL to Gabriel Almeida for me?"<br>Turn 2<br>assistant get_contacts(filter_by_name="Gabriel Almeida")<br>Turn 3 [{ "contact_id": "ctc_8125", "name": "Gabriel Almeida",<br>tool "pix_key": "Gabriel.Almeida@fakegmail.com", ... }]<br>Turn 4<br>assistant make_pix_to_contact(contact_id="ctc_8125", amount=890)<br>Turn 5<br>tool Pix of 890 BRL successfully sent to ctc_8125 (Gabriel Almeida)<br>Turn 6<br>"I sent the Pix of 890 BRL to Gabriel Almeida. [...]"<br>assistant<br>Legend:<br>user assistant tool temporal sequence<br>**----- End of picture text -----**<br>


Figure 12: Sample from pix-bench benchmark. Note that the example was translated to English for the paper. Original language is Portuguese. 
...
|gemini-2.5-fash-lite|0.1|0.4|Google|
|gpt-5-mini|0.25|2|OpenAI|
|sabi´a-3.1|0.93|1.85|Maritaca AI|
|sabi´a-4|0.93|3.7|Maritaca AI|
|Qwen3-235b-instruct|0.23|0.92|Alibaba|
|gpt-4.1|2|8|Openai|
|gpt-5.2 (instant)|1.75|14|Openai|
|gpt-5.2 (high)|1.75|14|Openai|
|gemini-3-pro (low)|2|12|Google|
|gemini-3-pro (high)|2|12|Google|
|kimi-k2-thinking|1.2|4|Together AI|
|deepseek-v3.2|0.28|0.42|Deepseek|



Table 4: Input and output prices used for evaluations, including the providers considered. 
