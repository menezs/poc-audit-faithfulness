...
OUTPUT_BASE = Path("./results/audit_with_chunks_direito_similar")
OUTPUT_BASE.mkdir(parents=True, exist_ok=True)

REGISTER_FILE = Path("./register/register_direito_similar.json")
CHUNKS_DIR = Path("./chunks/direito")
    ...
    for entry in register_data:
        ...
        for idx, chunk in enumerate(chunks):
            if idx >= len(documents):
                print(f"  Chunk {idx + 1}/{len(chunks)} sem documento correspondente, pulando")
                continue

            doc_path = documents[idx]
            print(f"  >> Chunk {idx + 1}/{len(chunks)} -> Documento: {doc_path}")

            doc_entry = get_or_create_doc_entry(answer_data, doc_path)

            if is_chunk_processed(doc_entry, idx):
                print(f"     Ja processado, pulando")
                continue
